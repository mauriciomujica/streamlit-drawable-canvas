...
def st_canvas(
    ...
    if background_image:
        ...
        # Create LayoutConfig object for compatibility with newer Streamlit versions
        from streamlit.elements.lib.layout_utils import LayoutConfig
        layout_config = LayoutConfig(width=width, height=height)
        background_image_url = st_image.image_to_url(
            background_image, layout_config, True, "RGB", "PNG", f"drawable-canvas-bg-{md5(background_image.tobytes()).hexdigest()}-{key}" 
        )
